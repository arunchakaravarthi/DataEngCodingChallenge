        ...
        public void ReadJsonFiles()
        {
            ...
            foreach (var jsonline in Lines)
            {
                ...
                {                
                    ...
                    var matches = regex.Matches(objItem.text);
                    if (matches.Count > 0)
                    {
                        foreach (Match m in matches)
                        {
                            contentLines += " #" + m.Value;
                        }
                        contentLines += "(timestamp: " + Created_at + ")\r\n";
                    } 
                }
            }
            string filepath = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), @"..\..\..\tweet_output\\"));
            System.IO.StreamWriter file = new System.IO.StreamWriter(filepath + "ft2.txt");
